...
CLRes <- read_xlsx("data_inSurvey_Response_Final.xlsx", sheetIndex = 1, header = TRUE, as.data.frame = TRUE, 
                   colClasses = (Response = "character"), stringsAsFactors = FALSE)

## Next load the survey answer for each participant and question
Values <- read.xlsx("Survey_Response_Final.xlsx", sheetIndex = 2, header = TRUE, as.data.frame = TRUE,
                    colClasses = (Response = "character"), stringsAsFactors = FALSE)


## Melt df for confidence level from wide format to long format for working with tidy
clres2 <- gather(CLRes, Person, Response, DJ:IK)
...
values2 <- gather(Values, Person, Response, DJ:IK)

## Join confidence level and response dataframes by question, species, and participant (person)
df <- inner_join(clres2, values2, by = c("Question", "Species", "Person"))
colnames(df) <- c("Question", "Species", "Person", "CL", "Value")
