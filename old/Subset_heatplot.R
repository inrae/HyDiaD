...
## To rank, relevel country, then arrange by both country and latitude and finaly define as a vector for use later
RankCL <- basin_area %>% 
...
species_cs <- FUNsubset(cliModel = Tmp_nit_cs, 
                         hsiModel = hsics_df, 
                         Dmax = Dmax)
species_cn <- FUNsubset(cliModel = Tmp_nit_cn,
                         hsiModel = hsicn_df, 
                         Dmax = Dmax)
species_no <- FUNsubset(cliModel = Tmp_nit_no,
                         hsiModel = hsino_df,
                         Dmax = Dmax)
...
## SR:
hp_species_cn <- species_cn %>% FUNheatplot(fillvar = .$SR,
                           scalelabel = paste0(Species, "\n" ,"Saturation Rate"))
hp_species_cs <- species_cs %>% FUNheatplot(fillvar = .$SR,
                              scalelabel = "Saturation Rate")
hp_species_no <- species_no %>% FUNheatplot(fillvar = .$SR,
                              scalelabel = "Saturation Rate")
## Density:
hp_species_cn_den <- species_cn %>% FUNheatplot(fillvar = .$Den,
                                  scalelabel = "Density")
hp_species_cs_den <- species_cs %>% FUNheatplot(fillvar = .$Den,
                                  scalelabel = "Density")
hp_species_no_den <- species_no %>% FUNheatplot(fillvar = .$Den,
                                  scalelabel = "Density")

species_cn %>% 
  filter(Basin == 'Garonne') %>% 
  select(Basin, Year, SR)
